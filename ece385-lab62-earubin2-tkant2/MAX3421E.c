...
//xilinx example uses length 12 buffers so i will too
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int test = 2;
	//for
	buff_write[0] = reg + test;
	buff_read[0] = 0;
	//write val via SPI
	buff_write[1] = val;
	buff_read[1] = 0;
	int transfer_code = XSpi_Transfer(&SpiInstance, buff_write, NULL, 2);
	//read return code from SPI peripheral (see Xilinx examples)
	//if return code != 0 print an error
	if(transfer_code)
	{
		xil_printf("Error, return code != 0 \n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for(int n = 0; n < nbytes; n++)
	{
		buff_write[n+1] = data[n];
	}
	int transfer_code = XSpi_Transfer(&SpiInstance, buff_write, NULL, nbytes+1);

	//int transfer_code = XSpi_Transfer(&SpiInstance, data, ReadBuffer, nbytes+1);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read val via SPI
	//read return code from SPI peripheral
	int transfer_code = XSpi_Transfer(&SpiInstance, buff_write, buff_read, 2);
	//BYTE val = buff_read[1];
	//if return code != 0 print an error
	if(transfer_code)
	{
		xil_printf("Error, return code != 0 \n");
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	//write reg via SPI
	typedef BYTE FunctionBuffer[nbytes+1];
	BYTE buff_read[nbytes+1];
	BYTE buff_write[nbytes+1];
	int test = 2;
	buff_write[0]= reg;
	...
	int transfer_code = XSpi_Transfer(&SpiInstance, buff_write, buff_read, nbytes+1);
	for(int n = 0; n < nbytes; n++)
	{
		data[n] = buff_read[n+1];

	}
	//read return code from SPI peripheral
	//int transfer_code = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, BUFFER_SIZE);
	//if return code != 0 print an error
	if(transfer_code)
	{
		xil_printf("Error, return code != 0 \n");
